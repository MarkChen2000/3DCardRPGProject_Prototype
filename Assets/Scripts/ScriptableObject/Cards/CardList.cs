...
public enum CardListType { BattleCardList, InventoryCrdList, PackCardPool }
...
public class CardList : ScriptableObject
{
    ...
    void OnEnable() // when this SO get refernece, sort and remove empty element in list first.
    {
        ...
        if ( _CradListType == CardListType.PackCardPool)// pack pool card should only have one card each.
        {
            RemoveCardAboveOne();
        }
    }
    ...
    private void RemoveCardAboveOne()
    {
        // It will remove all the repeated card.
    }
